...
N64.pow = function pow(N64, num, exp) {
  return new N64().fromInt(num).ipown(exp);
};

N64.shift = function shift(N64, num, bits) {
  return new N64().fromInt(num).ishln(bits);
};
...
U64.fromJSON = function fromJSON(json) {
  return new U64().fromJSON(json);
};
...
I64.fromJSON = function fromJSON(json) {
  return new I64().fromJSON(json);
};
